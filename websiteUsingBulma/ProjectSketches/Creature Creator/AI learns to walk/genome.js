class Genome {
    ...
    fullyConnect(innovationHistory) {
        ...
        for (var i = 0; i < this.inputs; i++) {
          for (var j = 0; j < this.outputs; j++) {
            let connectionInnovationNumber = this.getInnovationNumber(innovationHistory, this.nodes[i], this.nodes[this.inputs + j]);
            this.genes.push(new connectionGene(this.nodes[i], this.nodes[this.inputs + j], random(-1, 1), connectionInnovationNumber));
          }
        }
        ...
        for(let i = 0 ; i< this.outputs; i++){
            let connectionInnovationNumber = this.getInnovationNumber(innovationHistory, this.nodes[this.nodes.length-1], this.nodes[this.inputs + i]);
            this.genes.push(new connectionGene( this.nodes[this.nodes.length-1], this.nodes[this.inputs + i], random(-1, 1), connectionInnovationNumber));
        }
    ...
    addConnection(innovationHistory) {
        ...
        const connectionInnovationNumber = this.getInnovationNumber(innovationHistory, this.nodes[randomNode1], this.nodes[randomNode2]);
    ...
    fullyConnected() {
        ...
        const nodesInLayers = []; //array which stored the amount of this.nodes in each layer
    ...
    crossover(parent2) {
        ...
        const childGenes = []; // new ArrayList<connectionGene>();//list of genes to be inherrited form the parents
        const isEnabled = []; // new ArrayList<Boolean>();
        //all inherited genes
        for (var i = 0; i < this.genes.length; i++) {
            let setEnabled = true; //is this node in the chlid going to be enabled

            const parent2gene = this.matchingGene(parent2, this.genes[i].innovationNo);
    ...
    printGenome() {
        console.log("Print genome  layers:" + this.layers);
    ...
    drawGenome(startX, startY, w, h) {
        ...
        const allNodes = []; //new ArrayList<ArrayList<Node>>();
        const nodePoses = []; // new ArrayList<PVector>();
        const nodeNumbers = []; // new ArrayList<Integer>();
        ...
        startX += ellipseSize / 2;
        startY += ellipseSize / 2;
        w -= ellipseSize / 2;
        h -= ellipseSize / 2;

        //split the this.nodes varo layers
        for (var i = 0; i < this.layers; i++) {
            const temp = []; // new ArrayList<Node>();
        ...
        for (var i = 0; i < this.layers; i++) {
            ...
            const x = startX + float((i + 1) * w) / float(this.layers + 1);
            for (var j = 0; j < allNodes[i].length; j++) { //for the position in the layer
                let y = 0;
                if (i !== 0)//if this is the final layer then do it differently
                    y = startY + float((j+1.0)* h) / float(allNodes[i].length+1.0);
                else
                    y = startY + float((j)* h) / float(allNodes[i].length-1.0);
        ...
        for (var i = 0; i < nodePoses.length; i++) {
            ...
            fill(200);
            noStroke();
            ellipse(nodePoses[i].x, nodePoses[i].y, ellipseSize, ellipseSize);
