...
function mousePressed() {
  ...
  if(!mouseIsOnScreen())
    return;
  mouseDownCount = 0;
  mouseDragFrom = createVector(mouseX,mouseY);
  dragging = true;
  dragFrom= createVector((mouseX+cameraX)/zoom,(mouseY+cameraY)/zoom);

  let newZoom = createVector((canvas.width/2+cameraX)/zoom,(canvas.height/2+cameraY)/zoom);
  if(mouseIsOnScreen()){

  }
}
function dragCamera(){
  let  newMousePos = createVector((mouseX+cameraX)/zoom,(mouseY+cameraY)/zoom);

  cameraX -= (newMousePos.x-dragFrom.x)*zoom;
  cameraY  -= (newMousePos.y - dragFrom.y)*zoom;
}
function mouseReleased(){
  dragging = false;
  if(mouseDownCount<20 && dist(mouseDragFrom.x,mouseDragFrom.y, mouseX,mouseY)<10){
...
function keyPressed() {
  ...
  let zoomRelativeTo = createVector((canvas.width/2+cameraX)/zoom,(canvas.height/2+cameraY)/zoom);
  if(mouseIsOnScreen()){
    zoomRelativeTo= createVector((mouseX+cameraX)/zoom,(mouseY+cameraY)/zoom);
  }
  ...
  let newZoom = createVector((canvas.width/2+cameraX)/zoom,(canvas.height/2+cameraY)/zoom);
  if(mouseIsOnScreen()){
    newZoom= createVector((mouseX+cameraX)/zoom,(mouseY+cameraY)/zoom);
  }


  cameraX -= (newZoom.x-zoomRelativeTo.x)*zoom;
  cameraY  -= (newZoom.y - zoomRelativeTo.y)*zoom;
